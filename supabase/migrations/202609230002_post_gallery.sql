-- Existing attachments remain private and become the first image of each gallery.
alter table public.post_media drop constraint post_media_post_id_key;
alter table public.post_media add column sort_order integer not null default 0 check(sort_order >= 0);
create unique index post_media_post_order on public.post_media(post_id, sort_order);
...
begin
 ...
 insert into public.post_media(post_id,storage_path,mime_type,size_bytes,alt_text,sort_order)

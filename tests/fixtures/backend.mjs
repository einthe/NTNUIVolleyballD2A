...
const port = 54329;
const serviceKey = "local-test-service-key-never-use-in-production";
const db = new PGlite();
...
async function embed(table, row, selection) {
  ...
  for (const field of splitSelection(selection)) {
    ...
    if (matches.length !== 1) throw new Error(`Ambiguous or missing relationship ${table}.${raw}`);
        ...
        try {
          ...
          if (path.startsWith("/rest/v1/rpc/")) {
            ...
            if (name === '"admin_users"')
              return send(
                res,
                200,
                (await db.query(`select to_jsonb(t) as row from public.admin_users() t`)).rows.map(
                  (r) => r.row,
                ),
              );
          ...
          if (path.startsWith("/rest/v1/")) {
            ...
            if (failures.get(operation) > 0) {
              ...
              return send(res, 400, { code: "TEST_QUERY_ERROR", message: "Injected test failure" });
            }
...
server.listen(port, "127.0.0.1", () => console.log(`Isolated SQL test adapter ready on ${port}`));
